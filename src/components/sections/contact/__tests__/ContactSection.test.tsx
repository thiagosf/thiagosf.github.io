...
describe('ContactSection', () => {
    ...
    it('calls onLinkClick when a link is clicked', async () => {
        ...
        render(<ContactSection contactLinks={contactLinks} onLinkClick={handleLinkClick} />)

        const githubLink = screen.getByRole('link', { name: /github\.com\/thiagosf/i })
        await user.click(githubLink)

        expect(handleLinkClick).toHaveBeenCalledWith(contactLinks[0].id, contactLinks[0].url)
    })

    it('displays an empty state when no links are provided', () => {
        render(<ContactSection contactLinks={[]} />)

        expect(screen.getByText(/no contact information provided/i)).toBeInTheDocument()
    })
    ...
    it('falls back to the globe icon when an unknown icon identifier is set', () => {
        ...
        expect(screen.getByTestId('contact-link-icon-cl-unknown')).toHaveAttribute('data-icon-name', 'globe')
    })
})
