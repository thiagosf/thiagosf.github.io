...
describe('HeroSection', () => {
    ...
    it('renders the introduction text', async () => {
        ...
        await waitFor(() => {
            ...
            const lastName = h2s.find(h => h.textContent?.replace(/\u00a0/g, ' ').includes('Silva'))
        ...
        const descriptionElement = screen.getByText((content, element) => {
            const hasText = (node: Element | null) => normalizeText(node?.textContent) === heroData.introduction.description
